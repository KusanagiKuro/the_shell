...
from the_shell import My_Shell
...
def get_command(shell):
    ...
    shell.command_history.append(shell.command)
...
def delete_character_in_line(shell, y, x):
    ...
    shell.command = (shell.command[:x - shell.prompt_length] +
                     shell.command[x - shell.prompt_length + 1:])
...
def print_str(window, string, end="\n"):
    ...
    Input:
        - window: The curses window that needs to be printed on
        - string: The string that will be printed on the window
    """
    window.addstr(string + end)


def modify_shell_on_resize(shell):
    new_size = shell.window.getmaxyx()
    pass
